...
这页只回答两件事：现在大家在聊什么，以及每个话题后来去了哪里。它不是承诺书，状态会随着讨论和实际测试变化。

## 状态

- **新鲜想法**（`idea`）：刚提出来，先收集真实场景。
- **聊着呢**（`discussing`）：有分歧，正在比较方案和代价。
- **准备做**（`planned`）：进入近期计划。
- **做出来了**（`shipped`）：已经可以试用或验证。
- **先放着**（`declined`）：现在不做，原因写在对应 issue 里。
...
目前还没有进入“准备做”或“做出来了”的话题。等第一轮讨论结束，维护者会在这里留下链接、结论和理由。

## 更新节奏

维护者通常每周集中整理一次；有明确进展时随时更新。想推动一个话题，最有用的做法是在原帖补充自己的使用场景，而不是单独再开一帖。
